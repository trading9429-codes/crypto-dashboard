...
function loadCache() {
  try {
    if (fs.existsSync(CACHE_FILE)) {
      ...
      avgVolumes = cache.avgVolumes || {};
      console.log("[CACHE] Restored avg volumes from disk");
    }
  } catch (err) {
...
async function fetch15mBaseline(symbol, days = DAYS) {
  ...
  // 3️⃣ Final fallback: ticker 24hr ÷ 96
  try {
...
function setupSymbolWS(symbol, avg) {
  const ws = new WebSocket(
    `wss://fstream.binance.com/ws/${symbol.toLowerCase()}@kline_15m`
  );
  ...
  ws.on("message", (msg) => {
    try {
      ...
      const trigger = vol >= avg * 3; // 3× rule
      ...
      if (trigger) {
        ...
        lastTriggers[symbol].unshift({
          ...
          volume: vol,
          avg,
          relVol: (vol / avg).toFixed(2),
          ltp_inr: closePrice * 83
        });
        lastTriggers[symbol] = lastTriggers[symbol].slice(0, TRIGGER_HISTORY);

        console.log(`[TRIGGER] ${symbol} at ${istTime}, vol=${vol}, avg=${avg.toFixed(2)}`);
  ...
  ws.on("close", () => {
    ...
    setTimeout(() => setupSymbolWS(symbol, avg), 5000);
  });
}
...
async function init() {
  ...
  await refreshBaselines();       // initial fetch
  scheduleDailyRefresh();         // schedule daily auto-refresh

  for (const symbol of symbols) {
    const avg = avgVolumes[symbol]?.avg;
    if (avg) setupSymbolWS(symbol, avg);
  }
}
